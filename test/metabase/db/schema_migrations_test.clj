      ...
      (let [user       (create-raw-user! (mt/random-email))
            ...
            collection (t2/insert-returning-pk! (t2/table-name Collection)
                                                {:name     "Silly Collection"
                                                 ...
                                                 :archived true})]
        (is (nil? (t2/select-one :model/Collection :id config/trash-collection-id)))
        (is (empty? (t2/select-fn-set :id :model/Dashboard :collection_id config/trash-collection-id)))
        (is (empty? (t2/select-fn-set :id :model/Card :collection_id config/trash-collection-id)))
        (is (empty? (t2/select-fn-set :id :model/Collection :location @#'collection/trash-path)))
        (migrate!)
        (is (set/subset? #{dash} (t2/select-fn-set :id :model/Dashboard :collection_id config/trash-collection-id)))
        (is (set/subset? #{card} (t2/select-fn-set :id :model/Card :collection_id config/trash-collection-id)))
